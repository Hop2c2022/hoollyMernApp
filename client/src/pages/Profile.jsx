...
const Profile = () => {
  ...
  const edit = async () => {
    if (newName.length != 0 && newName.length < 13) {
      const res = await axios.patch(`hoolly-mern-api.vercel.app/${localStorage.getItem('id')}`, {
        name: newName,
      });
      if (res?.status == 200) {
  ...
  const info = async () => {
    ...
    const res2 = await axios.get(`hoolly-mern-api.vercel.app/${localStorage.getItem('id')}`);
