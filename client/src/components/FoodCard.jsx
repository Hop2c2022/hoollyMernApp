...
const FoodCard = () => {
  ...
  const orders = async () => {
    if (val == 'breakfast') {
      const res = await axios.get('https://hoolly-mern-app.vercel.app/orders/breakfast');
      setFoods(res?.data?.result);
      setLoading(false);
    } else if (val == 'dinner') {
      const res = await axios.get('https://hoolly-mern-app.vercel.app/orders/dinner');
      setFoods(res?.data?.result);
      setLoading(false);
    } else if (val == 'lunch') {
      const res = await axios.get('https://hoolly-mern-app.vercel.app/orders/lunch');
      setFoods(res?.data?.result);
      setLoading(false);
    } else {
      const res = await axios.get(`https://hoolly-mern-app.vercel.app/restaurant/${val}`);
